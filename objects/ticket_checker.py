...
class TicketChecker:
    ...
    def is_oversold(self):
        ...
        event_ticket_total = EventModel().get_number_of_tickets_by_event(self.event_id)
        event_capacity = int(EventModel().get_event(self.event_id).get('capacity'))
        ticket_type_ticket_total = EventModel().get_number_of_tickets_by_ticket_type(self.ticket_type_id)
